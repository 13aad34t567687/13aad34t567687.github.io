...
console.log("FluidCursor script executing...", { canvasFound: !!canvas })
...
if (canvas) {
    ...
    function getSupportedFormat(gl: any, internalFormat: number, format: number, type: number) {
        if (!supportRenderTextureFormat(gl, internalFormat, format, type)) {
            ...
            if (internalFormat === gl2.RG16F) return getSupportedFormat(gl, gl2.RGBA16F, gl2.RGBA, type)
            return null
        }
    ...
    window.addEventListener('touchend', (e: TouchEvent) => {
        ...
        for (let i = 0; i < touches.length; i++) {
            for (let j = 0; j < pointers.length; j++) {
                if (touches[i].identifier === pointers[j].id) updatePointerUpData(pointers[j])
            }
        }
    })
