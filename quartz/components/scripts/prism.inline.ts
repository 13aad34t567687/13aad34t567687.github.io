...
function initPrism() {
    ...
    const config = {
        ...
        glow: 1,
        offset: { x: 0, y: 0 },
        noise: 0, // Set to 0 to remove particle feel
        transparent: true,
        scale: 3.6,
        hueShift: 0,
        colorFrequency: 1,
    ...
    const HUE = config.hueShift;
    const CFREQ = Math.max(0.0, config.colorFrequency);
    const BLOOM = Math.max(0.0, config.bloom);
    const RSX = 1;
    const RSY = 1;
    const RSZ = 1;
    const TS = Math.max(0, config.timeScale);
    const HOVSTR = Math.max(0, config.hoverStrength);
    const INERT = Math.max(0, Math.min(1, config.inertia));
  ...
  `;
    ...
    } else if (config.animationType === '3drotate') {
        program.uniforms.uUseBaseWobble.value = 0;
    } else {
...
document.addEventListener("nav", initPrism)
